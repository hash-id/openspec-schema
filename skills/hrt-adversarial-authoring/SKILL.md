...
3. **Resolve.** The orchestrating session shows `Required Changes` to the user, one at a time if there's more than one, recommended resolution first, apply only what the user confirms or adjusts. `Suggested Improvements` and `Risks and Open Questions` are surfaced but don't block.
